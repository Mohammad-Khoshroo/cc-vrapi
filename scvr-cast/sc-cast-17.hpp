...
// Target: C++17, SystemC (IEEE 1666)
// ============================================================================

#include <systemc>         // SystemC core and data types (sc_core, sc_dt, sc_lv, SC_REPORT_*)
#include <type_traits>     // type traits: enable_if, is_integral, is_same, etc.
...
namespace sc_cast
{
    ...
    // -------------------- is_sc_logiclike (union of all three) --------------------
    template <typename T>
    struct is_sc_logiclike
    ...
    template <int W>
    struct sc_lv_traits<sc_dt::sc_bv<W>>
    {
        static constexpr int width = W;
        using type = sc_bv<W>;
    };
    ...
    template <>
    struct sc_lv_traits<sc_dt::sc_logic>
    {
    ...
    namespace detail
    {
        ...
        inline LV make_unknown()
        {
            constexpr int W = sc_lv_traits<LV>::width;
            if constexpr (is_sc_bv<LV>::value)
                return LV(std::string(W, '0').c_str());
            else if constexpr (is_sc_lv<LV>::value)
        ...
        // For sc_logic, truncation is a no-op (we take .back() later).
        template <typename LV>
        inline std::string adjust_bitstr(std::string bitstr,
                                         int WIDTH,
                                         bool is_data,
                                         const char* type_label)
        {
            ...
            if (sz > WIDTH)
            {
                ...
                // sc_logic: no truncation needed; .back() is taken later
            }
            else if (sz < WIDTH)
            {
        ...
        template <typename LV>
        inline LV from_bitstr(const std::string& bitstr)
        {
            if constexpr (is_sc_logic<LV>::value)
            {
    ...
    // Identity: LV → LV
    template <typename LV>
    typename std::enable_if<sc_cast::is_sc_logiclike<LV>::value, LV>::type
    ...
    typename std::enable_if<std::is_integral<T>::value && !std::is_same<T, bool>::value
                            && sc_cast::is_sc_logiclike<LV>::value, LV>::type
    ...
    template <typename LV>
    typename std::enable_if<sc_cast::is_sc_logiclike<LV>::value
                            && !sc_cast::is_sc_logic<LV>::value, LV>::type
    ...
    typename std::enable_if<std::is_same<T, bool>::value
                            && sc_cast::is_sc_logiclike<LV>::value, T>::type
        sc_lv_cast(const LV& lv);
    ...
    template <typename T = sc_logic, typename LV>
    typename std::enable_if<sc_cast::is_sc_logiclike<LV>::value
                            && !sc_cast::is_sc_logic<LV>::value
                            && std::is_same<T, sc_logic>::value, T>::type
        sc_lv_cast(const LV& lv);
    ...
    {
        // mode_view is compared directly as string_view (no std::string copy)
        ...
        else
        {
            ...
            else
            {
                ...
                // (drops inputTypeBitSize - WIDTH bits from the LSB side)
                if (WIDTH >= inputTypeBitSize)
                {
                    // No truncation needed; value fits entirely
        ...
        sc_lv_cast(float value)
    {
        ...
        if constexpr (is_sc_logic<LV>::value)
        {
            // WIDTH must be 1 here, but float needs 32 bits — error
        ...
        sc_lv_cast(std::string_view input_str, std::string_view mode_view, int base)
    {
        ...
        // Remove whitespace and warn (use safe_isspace to handle signed chars)
        ...
        if (size >= 3 && c0 == '0' && (c1 == 'x' || c1 == 'X'))
        {
            ...
            if (bitsNum > WIDTH)
            {
                if constexpr (!is_sc_logic<LV>::value)
                {
                    ...
                    // Truncate hex digits from the MSB side
            ...
            else if (bitsNum < WIDTH)
            {
                ...
                if (is_data)
                    hex_str = std::string(padDigits, pad) + hex_str;
                else // address
        ...
        if (size >= 2 && c0 == '0' && detail::safe_isdigit(c1))
        {
            ...
            else if (bitsNum < WIDTH)
            {
                ...
                if (is_data)
                    oct_str = std::string(padDigits, pad) + oct_str;
                else // address
    ...
    T string_cast(std::string_view str_view, std::string_view mode_view, int base)
    {
        ...
        if constexpr (sc_cast::is_sc_logiclike<T>::value)
        {
            using LV = T;
        ...
        else if constexpr (std::is_integral<T>::value)
        {
            ...
            {
                try { return static_cast<T>(std::stoull(str, nullptr, 2)); }
                catch (...) {
                    SC_REPORT_WARNING("string_cast", "Binary parse error. Returning 0.");
                    return static_cast<T>(0);
                }
            }
            // Hex: "0x..."
            ...
            {
                try { return static_cast<T>(std::stoull(str, nullptr, 16)); }
                catch (...) {
                    SC_REPORT_WARNING("string_cast", "Hex parse error. Returning 0.");
                    return static_cast<T>(0);
                }
            }
            // Octal: "0..."
            ...
            {
                try { return static_cast<T>(std::stoull(str, nullptr, 8)); }
                catch (...) {
                    SC_REPORT_WARNING("string_cast", "Octal parse error. Returning 0.");
                    return static_cast<T>(0);
                }
            }

            // Decimal fallback
            try { return static_cast<T>(std::stoull(str)); }
            catch (...) {
                SC_REPORT_WARNING("string_cast", "Decimal parse error. Returning 0.");
                return static_cast<T>(0);
            }
        }
        else if constexpr (std::is_floating_point<T>::value)
        {
            ...
            {
                try { return static_cast<T>(std::stod(str)); }
                catch (...) {
                    SC_REPORT_WARNING("string_cast", "Double parse error. Returning 0.");
                    return static_cast<T>(0);
                }
            }
            else if constexpr (std::is_same<T, float>::value)
            {
                try { return static_cast<T>(std::stof(str)); }
                catch (...) {
                    SC_REPORT_WARNING("string_cast", "Float parse error. Returning 0.");
                    return static_cast<T>(0);
                }
            }
        }
