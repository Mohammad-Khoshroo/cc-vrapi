...
namespace cc_vrwrapper
{
    ...
    sc_cast(double value)
    {
        ...
        // string → sc_lv
    template <typename LV, typename S>
    std::enable_if_t<is_sc_lv<LV>::value && is_string_like<S>::value, LV>
    sc_cast(S&& input_str,
    ...
    // ===== FROM sc_lv<W> (to primitive types only) =====
    ...
    sc_cast(const LV& lv) { return lv.to_string(); }
