...
namespace cc_vrwrapper
{
    ...
    template <ScBv BV>
    BV sc_cast(const BV& bv) { return bv; }

    template <ScBv BV>
    BV sc_cast(bool value)
    {
    ...
    BV sc_cast(T value, std::string_view mode_view = "data", bool MSB = false)
    {
    ...
    BV sc_cast(double value)
    {
        ...
        // string → sc_bv
    template <ScBv BV, StringLike S>
    BV sc_cast(S&& input_str,
    ...
    // ===== FROM sc_bv<W> (only to primitive types) =====

    template <typename T = bool, ScBv BV>
        requires std::same_as<T, bool>
    T sc_cast(const BV& bv) { return bv.get_bit(0); }

    template <std::integral T, ScBv BV>
        requires (!std::same_as<T, bool>)
    T sc_cast(const BV& bv)
    {
    ...
    T sc_cast(const BV& bv) { return bv.to_string(); }
