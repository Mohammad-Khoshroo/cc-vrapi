...
namespace sc_cast
{
    ...
    // Identity
    template <ScBv BV>
    BV sc_lv_cast(const BV& bv) { return bv; }

    // bool → sc_bv
    template <ScBv BV>
    BV sc_lv_cast(bool value)
    {
    ...
    template <ScBv BV, std::integral T>
        requires (!std::same_as<T, bool>)
    BV sc_lv_cast(T value, std::string_view mode_view = "data", bool MSB = false)
    {
    ...
    // float → sc_bv (C++20: std::bit_cast)
    template <ScBv BV>
    BV sc_lv_cast(float value)
    {
        constexpr int WIDTH = sc_lv_traits<BV>::width;
        if (WIDTH < 32)
    ...
    // double → sc_bv (C++20: std::bit_cast)
    template <ScBv BV>
    BV sc_lv_cast(double value)
    {
        constexpr int WIDTH = sc_lv_traits<BV>::width;
        if (WIDTH < 64)
    ...
    // string → sc_bv
    template <ScBv BV>
    BV sc_lv_cast(std::string_view input_str,
    ...
    // sc_logic → sc_bv
    template <ScBv BV>
    BV sc_lv_cast(const sc_logic& lg)
    {
        constexpr int WIDTH = sc_lv_traits<BV>::width;
        if (lg.is_01())
    ...
    // sc_lv → sc_bv
    template <ScBv BV, int W2>
    BV sc_lv_cast(const sc_lv<W2>& lv)
    {
        constexpr int WIDTH = sc_lv_traits<BV>::width;
        if (!lv.is_01())
    ...
    // sc_int family → sc_bv
    template <ScBv BV, ScIntLike INT>
    BV sc_lv_cast(const INT& val)
    {
    ...
    // sc_bv → bool
    template <typename T = bool, ScBv BV>
        requires std::same_as<T, bool>
    T sc_lv_cast(const BV& bv) { return bv.get_bit(0); }

    // sc_bv → integral (not bool)
    template <std::integral T, ScBv BV>
        requires (!std::same_as<T, bool>)
    T sc_lv_cast(const BV& bv)
    {
    ...
    // sc_bv → float (C++20: std::bit_cast)
    template <typename T = float, ScBv BV>
    ...
    // sc_bv → double (C++20: std::bit_cast)
    template <typename T = double, ScBv BV>
    ...
    // sc_bv → string
    template <typename T = std::string, ScBv BV>
        requires std::same_as<T, std::string>
    T sc_lv_cast(const BV& bv) { return bv.to_string(); }

    // sc_bv → sc_logic (LSB)
    template <typename T = sc_logic, ScBv BV>
