...
void test_json_logging()
{
    ...
    while (std::getline(f, line)) {
        if (line.front() == '{' && line.back() == '}') json_lines++;
    }
...
void test_hierarchy_capture()
{
    std::cout << "\n=== Test 12: Hierarchy capture ===\n";

    rpt::config::set_capture_hierarchy(true);
    rpt::config::set_log_file_path("logs/test_hierarchy.log");
    rpt::stats::reset();
    rpt::handler::init_report_handler();

    TestModule tm("test_mod");
    sc_start(1, SC_NS);
