...
struct Config {
    ...
    // CHANGED: renamed max_severity → min_severity, default SC_FATAL → SC_INFO
    // This is the MINIMUM severity to display (acts as a threshold).
    // Reports with severity < min_severity are suppressed.
    // Note: filters::enabled_severities provides an additional, finer-grained
    //       filter (e.g. enable SC_INFO + SC_WARNING but disable SC_ERROR).
    //       A report must pass BOTH filters to be logged.
...
// CHANGED: renamed set_max_severity → set_min_severity
